...
export function NotificationsControl() {
  ...
  useEffect(() => {
    let active = true;
    void fetch("/api/failure-cases/FC-2026-0047", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ notifications: PersistedNotification[] }> : null)
      .then((payload) => {
