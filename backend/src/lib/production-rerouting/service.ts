...
export async function runProductionRerouting(input: { recoveryTimeEstimateId: string; correlationId: string; failureCaseExternalId: string }) {
  const [context] = await db.select({ estimateId: recoveryTimeEstimates.id, correlationId: recoveryTimeEstimates.correlationId, failureCaseId: failureCases.id, failureCaseExternalId: failureCases.externalId, sourceWorkstationId: failureCases.workstationId }).from(recoveryTimeEstimates).innerJoin(failureCases, eq(recoveryTimeEstimates.failureCaseId, failureCases.id)).where(eq(recoveryTimeEstimates.id, input.recoveryTimeEstimateId)).limit(1);
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      for (const job of jobs) {
        ...
        const rationale = { operationCode: job.operationCode, toolingCode: job.toolingCode, requiredSkill: job.requiredSkill, estimatedLoadPercent: job.estimatedLoadPercent, candidateCodes: candidates.map((candidate) => candidate.code) };
        if (!selected) {
