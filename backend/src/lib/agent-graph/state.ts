...
export const RecoveryGraphStateAnnotation = Annotation.Root({
  ...
  procurementAutomationResultId: Annotation<string | null>,
  procurementRequestId: Annotation<string | null>,
  vendorNotificationId: Annotation<string | null>,
  procurementAutomationOutcome: Annotation<"requisition_created" | "no_eligible_vendor" | null>,
  maintenanceWorkOrderId: Annotation<string | null>,
  recoveryTimeEstimateId: Annotation<string | null>,
  rerouteDecisionIds: Annotation<string[]>({ reducer: (_current, update) => update, default: () => [] }),
  reroutingOutcome: Annotation<"rerouted" | "requires_intervention" | null>,
  deliveryImpactIds: Annotation<string[]>({ reducer: (_current, update) => update, default: () => [] }),
  deliveryImpactOutcome: Annotation<"calculated" | "no_linked_shipment" | null>,
  finalNotificationIds: Annotation<string[]>({ reducer: (_current, update) => update, default: () => [] }),
  finalNotificationOutcome: Annotation<"queued" | "not_required" | null>,
...
export function initialRecoveryGraphState(input: { correlationId: string; graphRunId: string; telemetrySourceEventId: string }): RecoveryGraphState {
  return {
    ...
    procurementAutomationResultId: null,
    procurementRequestId: null,
    vendorNotificationId: null,
    procurementAutomationOutcome: null,
    maintenanceWorkOrderId: null,
    recoveryTimeEstimateId: null,
    rerouteDecisionIds: [],
    reroutingOutcome: null,
    deliveryImpactIds: [],
    deliveryImpactOutcome: null,
    finalNotificationIds: [],
    finalNotificationOutcome: null,
