...
export async function finalStakeholderNotificationNode(state: RecoveryGraphState): Promise<RecoveryGraphUpdate> {
  ...
  const patch: RecoveryGraphUpdate = { finalNotificationIds: result.notifications.map((notification) => notification.id), finalNotificationOutcome: result.outcome, workflowStatus: "COMPLETED" };
