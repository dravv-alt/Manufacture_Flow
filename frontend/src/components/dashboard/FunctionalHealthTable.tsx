...
type Metric = { powerKw: number; oeePercent: number; cycleTimeSeconds: number; outputPerHour: number; defectRatePercent: number; estimatedRulDays: number; operatorId: string; firmwareVersion: string; networkPingMs: number; lastMaintenanceAt: string; nextMaintenanceAt: string };
type Telemetry = { temperature: number; vibration: number; anomalySeverity: string };
const headers = ["WS ID", "Type", "Status", "Temp (C)", "Vib (mm/s)", "Power (kW)", "OEE (%)", "Cycle Time (s)", "Output/hr", "Defect Rate (%)", "Last Maint.", "Next Maint.", "Est. RUL (Days)", "Operator ID", "Firmware", "Network Ping (ms)", "Anomaly Score", "Action"];
const dash = "—";

export function FunctionalHealthTable({ workstations, onOpenStation }: { workstations: readonly Workstation[]; onOpenStation: (id: string) => void }) {
