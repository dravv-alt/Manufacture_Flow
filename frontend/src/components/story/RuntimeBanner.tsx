...
export function RuntimeBanner() {
  ...
  useEffect(() => {
    if (process.env.NEXT_PUBLIC_OPERATIONS_MODE !== "backend" || runtime !== "demo" || !currentUser) return;
    const tick = async () => {
      if (busy.current || document.visibilityState === "hidden") return;
      busy.current = true;
      try {
        const response = await apiFetch("/api/demo-control/trigger-telemetry", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ scenario: demoScenario, mode: "tick" })
        }, "demo");
        if (response.ok) await refreshRef.current();
      } catch { /* demo backend offline ignored */ } finally {
        busy.current = false;
      }
    };
    const timer = window.setInterval(() => void tick(), 10_000);
    return () => window.clearInterval(timer);
  }, [currentUser?.email, demoScenario, runtime]);
  if (process.env.NEXT_PUBLIC_OPERATIONS_MODE !== "backend") return null;
