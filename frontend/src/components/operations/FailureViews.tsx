...
function AuditTrail({ caseId }: { caseId: string }) {
  ...
  return (
    <section className="rounded-[2rem] border border-[#ddd6ce] bg-white p-7 shadow-[0_22px_40px_rgba(0,0,0,0.035)]">
      <div>
        <p className="text-xs font-semibold tracking-[0.06em] text-[#5d595d]">IMMUTABLE AUDIT HISTORY</p>
        <h2 className="mt-2 text-2xl font-semibold">Recorded workflow actions</h2>
      </div>
      <div className="mt-5 space-y-3">
        {displayEvents.slice(0, 6).map((event) => (
          <div key={event.id} className="flex items-start justify-between gap-4 rounded-xl bg-[#f5f2ef] p-4">
            <div>
              <p className="text-sm font-semibold">{event.eventType.replaceAll("_", " ")}</p>
              <p className="mt-1 text-xs text-[#615d60]">{event.actor}</p>
            </div>
