...
export function PersistedPreferences() {
  ...
  const save = async () => { if (!currentUser) { update({ reducedMotion: state.reducedMotion }); setError(""); setSaved(true); return; } const response = await apiFetch("/api/preferences", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reducedMotion: state.reducedMotion }) }); if (!response.ok) { setError("Preferences could not be saved."); return; } setError(""); setSaved(true); };
  return <div className="mt-5"><p className="text-sm text-muted-foreground">Reduced motion is saved with this browser demo.</p>{error ? <p role="alert" className="mt-2 text-sm text-destructive">{error}</p> : null}<Button className="mt-3" onClick={() => void save()}>{saved ? <Check data-icon="inline-start" /> : null}{saved ? "Preferences saved" : "Save preferences"}</Button></div>;
}
