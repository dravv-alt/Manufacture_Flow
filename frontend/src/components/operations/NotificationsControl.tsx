...
export function NotificationsControl() {
  ...
  const backendNotices = activeCase?.notifications?.map((notice) => ({
    id: notice.id,
    title: notice.subject,
    detail: `${notice.recipientRole} via ${notice.channel.replace("_", " ")}.`,
    status: notice.state as NotificationStatus,
    href: `/failure/${activeCase.failureCase.externalId || "FC-2026-0047"}`,
  }));
  ...
  const defaultAttempts: NotificationAttempt[] = [
    { id: "ATT-01", notificationId: "NT-121", attemptNumber: 1, state: "failed", actor: "Carrier Dispatch API Gateway", detail: "HTTP 504 Gateway Timeout on logistics webhook", occurredAt: new Date(Date.now() - 3600000).toISOString() },
    { id: "ATT-02", notificationId: "NT-117", attemptNumber: 1, state: "acknowledged", actor: "Warehouse Terminal WH-A", detail: "Scanned and acknowledged by Operator R-10023", occurredAt: new Date(Date.now() - 7200000).toISOString() },
  ...
  return (
    <main className="px-5 py-7 md:px-8 md:py-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        <section className="flex flex-col justify-between gap-5 border-b border-border pb-7 md:flex-row md:items-end">
          <div>
            <div className="flex items-center gap-3">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-0.5 font-mono text-[10px] font-semibold text-emerald-600 dark:text-emerald-400"><span className="size-1.5 rounded-full bg-emerald-500 animate-pulse" />EVENT DISPATCHER ONLINE</span>
              <span className="font-mono text-xs text-muted-foreground">DELIVERY CENTER</span>
            </div>
            <h1 className="mt-4 font-heading text-4xl font-semibold tracking-[-0.04em]">Notifications and delivery state.</h1>
            <p className="mt-3 max-w-2xl text-base leading-7 text-muted-foreground">Every alert is linked to its operational record. A failed delivery is visible and retryable; an acknowledgement is persisted in the audit timeline.</p>
          </div>
          <Badge variant="destructive">{notices.filter((notice) => notice.status === "failed").length} FAILED</Badge>
        </section>
        {commandError ? (
          <div role="alert" className="flex items-center justify-between gap-3 rounded-md border border-destructive bg-destructive/5 px-4 py-3 text-sm text-destructive">
            <span>{commandError}</span>
            <button className="underline" onClick={clearCommandError}>Dismiss</button>
          </div>
        ) : null}
        <div className="flex flex-wrap gap-2">
          {(["all", "unread", "failed", "acknowledged"] as Filter[]).map((option) => (
        ...
        <Card data-story="notification-result">
          <CardHeader>
            <CardTitle className="font-heading">Delivery records</CardTitle>
            <CardDescription>{visible.length} visible record{visible.length === 1 ? "" : "s"}</CardDescription>
          </CardHeader>
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {notice.status === "failed" ? (
                        ...
                        <Button size="sm" disabled={pendingCommand === "acknowledge_notification"} onClick={() => update(notice, "acknowledged")}>
                          <Check data-icon="inline-start" />Acknowledge
                        </Button>
                      ) : null}
                    </div>
                  </div>
                  {history.length ? (
                    <div className="border-t border-border pt-3">
                      ...
                      <div className="mt-2 space-y-2">
                        {history.map((attempt) => (
                          <div key={attempt.id} className="flex flex-wrap justify-between gap-2 text-xs text-muted-foreground">
                            <span>#{attempt.attemptNumber} · {attempt.detail} · {attempt.actor}</span>
                            <time>{new Date(attempt.occurredAt).toLocaleString()}</time>
                          </div>
                        ))}
                      </div>
                    </div>
