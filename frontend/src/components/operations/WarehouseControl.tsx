...
export function WarehouseControl() {
  ...
  const stock = inventory?.onHand ?? (state.inventoryAvailable || state.bearingReserved ? 3 : 2);
  const reservedCount = inventory?.reserved ?? (state.bearingReserved ? 1 : 0);
  const remaining = Math.max(0, stock - reservedCount);
  const available = remaining > 0 || state.bearingReserved;
  const reserved = state.bearingReserved;

  const plantSpares = [
    { code: "BRG-10023", name: "Deep Groove Spindle Bearing (SKF 6205-2RSH)", location: "WH-A / Rack B-14", qty: remaining + reservedCount, reserved: reservedCount, status: "Active Recovery", lot: "LOT-2026-B812" },
    { code: "SNS-88410", name: "Triaxial Piezoelectric Accelerometer (IFM VSA001)", location: "WH-A / Bin C-02", qty: 6, reserved: 0, status: "Nominal Stock", lot: "LOT-2026-A104" },
    { code: "BLT-44021", name: "High-Torque Drive Timing Belt (Gates GT3 8M)", location: "WH-B / Rack D-07", qty: 12, reserved: 2, status: "Nominal Stock", lot: "LOT-2025-D990" },
    { code: "LUB-99102", name: "Ultra-Speed Spindle Grease (Kluber Isoflex NBU 15)", location: "WH-A / Chem-01", qty: 8, reserved: 0, status: "Nominal Stock", lot: "LOT-2026-L441" },
    { code: "PLC-77210", name: "Beckhoff EtherCAT Bus Coupler (EK1100)", location: "WH-B / Bin E-11", qty: 3, reserved: 1, status: "Low Stock Alert", lot: "LOT-2025-E012" },
  ];
    ...
    <main className="px-5 py-7 md:px-8 md:py-10">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <section className="flex flex-col gap-5 border-b border-border pb-7">
          <div className="flex flex-wrap items-center gap-3"><span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-0.5 font-mono text-[10px] font-semibold text-emerald-600 dark:text-emerald-400"><span className="size-1.5 rounded-full bg-emerald-500" />SIMULATED INVENTORY</span><span className="font-mono text-xs text-muted-foreground">INVENTORY CONTROL / FAILURE CASE FC-2026-0047</span></div>
          <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div className="max-w-3xl"><h1 className="font-heading text-4xl font-semibold tracking-[-0.04em] md:text-5xl">Make the spare decision explicit.</h1><p className="mt-3 text-base leading-7 text-muted-foreground">Reserve BRG-10023 for the WS-102 bearing replacement or route the unavailable branch to a procurement draft.</p></div><span className="rounded-full border border-border bg-muted px-3 py-1 font-mono text-xs font-semibold">WMS REPOSITORY · WH-A &amp; WH-B</span></div>
        </section>

        <section data-story="inventory-result" className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <Card data-story="reserve-bearing"><CardHeader><CardTitle className="font-heading">Reservation control</CardTitle><CardDescription>Links this spare to FC-2026-0047 and the pending WS-102 work order.</CardDescription></CardHeader><CardContent className="flex flex-col gap-4">{commandError ? <div role="alert" className="rounded-md border border-destructive bg-destructive/5 p-3 text-sm text-destructive"><div className="flex items-center justify-between gap-3"><span>{commandError}</span><button className="underline" onClick={clearCommandError}>Dismiss</button></div></div> : null}{reserved ? <div className="rounded-md bg-muted p-4"><p className="flex items-center gap-2 text-sm font-semibold"><Check className="size-4" /> Bearing reserved</p><p className="mt-1 text-sm leading-6 text-muted-foreground">One simulated bearing is held for WO-WS102-081.</p><Button asChild className="mt-3"><Link href="/maintenance">Continue to maintenance</Link></Button></div> : available ? <AlertDialog><AlertDialogTrigger asChild><Button disabled={pendingCommand === "reserve_part"}><PackageCheck data-icon="inline-start" />{pendingCommand === "reserve_part" ? "Reserving bearing..." : "Reserve 1 bearing"}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Reserve BRG-10023?</AlertDialogTitle><AlertDialogDescription>This will hold one simulated bearing for WS-102 failure case FC-2026-0047. It does not place a purchase order.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction data-story="reserve-bearing-confirm" onClick={() => runWorkflowCommand({ type: "reserve_part", quantity: 1 })}>Confirm reservation</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : <div className="flex flex-col gap-3 rounded-md border border-destructive p-4"><p className="flex items-center gap-2 text-sm font-semibold text-destructive"><AlertTriangle className="size-4" /> Stock is insufficient</p><p className="text-sm leading-6 text-muted-foreground">No bearing can be reserved. Continue to a draft procurement request; financial approval remains outside this app.</p><Button asChild><Link href="/procurement">Open procurement draft</Link></Button></div>}{state.procurementState === "acknowledged" && <Button variant="outline" disabled={pendingCommand === "receive_part"} onClick={() => void runWorkflowCommand({ type: "receive_part" })}>Receive replenishment</Button>}<Link href="/failure/FC-2026-0047" className="text-sm font-medium underline-offset-4 hover:underline">Back to failure control</Link></CardContent></Card>
        </section>
        ...
        <Card><CardHeader><CardTitle className="font-heading">Plant Critical Spares Inventory</CardTitle><CardDescription>Real-time tracked components across Warehouse A and Warehouse B storage racks.</CardDescription></CardHeader><CardContent><div className="overflow-x-auto rounded-xl border border-border"><table className="w-full text-left text-sm"><thead className="border-b border-border bg-muted/50 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground"><tr><th className="p-3">Part Code</th><th className="p-3">Component Description</th><th className="p-3">Location</th><th className="p-3">Lot #</th><th className="p-3">On Hand</th><th className="p-3">Reserved</th><th className="p-3">Status</th></tr></thead><tbody className="divide-y divide-border">{plantSpares.map((spare) => <tr key={spare.code} className="hover:bg-muted/30"><td className="p-3 font-mono font-bold text-xs">{spare.code}</td><td className="p-3 font-medium">{spare.name}</td><td className="p-3 text-xs text-muted-foreground">{spare.location}</td><td className="p-3 font-mono text-[11px] text-muted-foreground">{spare.lot}</td><td className="p-3 font-mono font-semibold">{spare.qty}</td><td className="p-3 font-mono text-muted-foreground">{spare.reserved}</td><td className="p-3"><Badge variant={spare.status === "Active Recovery" ? "secondary" : spare.status === "Low Stock Alert" ? "warning" : "outline"} className="text-[10px]">{spare.status}</Badge></td></tr>)}</tbody></table></div></CardContent></Card>
      </div>
    </main>
  );
}
